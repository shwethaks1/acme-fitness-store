...
def verify_token(token):
    ...
    if auth_mode == 2:
        ...
        if token == "":
            ...
            app.logger.info("verify_token_payload is %s", verify_token_payload)
            app.logger.info("headers are %s", headers)
            r = requests.post(verify_token_url, headers=headers, data=verify_token_payload)
            app.logger.info("response content is %s", r.content)
            app.logger.info("response content as string is %s", str(r.content))
            #if r.status_code == 200:
...
def get_items(userid, span_c):
    ...
    with cart_tracer.start_span(function_name, child_of=span_c) as span:
        ...
        with cart_tracer.start_span('/redis/extract/get', child_of=span) as redis_span:
            for key in rConn.scan_iter("*"):
                app.logger.info("rConn keys are" + key.decode("utf-8"))
